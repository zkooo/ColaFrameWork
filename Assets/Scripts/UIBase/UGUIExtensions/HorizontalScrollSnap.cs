...
namespace UnityEngine.UI.Extensions
{
    ...
    public class HorizontalScrollSnap : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        private GameObject removedGo = null;
        ...
        void Awake()
        {
            ...
            if (PageStep == 0)
            {
                PageStep = (int)_scroll_rect.GetComponent<RectTransform>().rect.width * 1;
            }
            DistributePages();
        ...
        private void DistributePages()
        {
            ...
            {
                GameObject temp = _screensContainer.transform.GetChild(i).gameObject;
                if (temp.activeSelf)
                {
                    RectTransform child = temp.GetComponent<RectTransform>();
                    currentXPosition = _offset + (i - inactiveGmo) * PageStep;
            ...
            _screens = _screensContainer.childCount - inactiveGmo;
        ...
        void OnValidate()
        {
            var childCount = gameObject.GetComponent<ScrollRect>().content.ActivedChildCount();
        ...
        {
            removedGo = null;
            RemoveChild(index, out removedGo);
            return removedGo;
        }
        ...
        public void RemoveChild(int index, out GameObject ChildRemoved)
        {
            ...
            if (_currentScreen > _screens - 1)
            {
                _currentScreen = Mathf.Max(0, _screens - 1);
            }
